...
from data_loader import get_face_data_as_object


fileLocation ='/fer2013/fer2013.csv'
face = get_face_data_as_object(fileLocation,output_column_index=0)

MODEL_NAME = 'em_convnet_6000'
...
def train(x, keep_prob, y_, train_step, loss, accuracy,
    ...
    with tf.Session() as sess:
        ...
        for step in range(NUM_STEPS):
            batch = face.train.next_batch(BATCH_SIZE)
            if step % 100 == 0:
        ...
        test_accuracy = accuracy.eval(feed_dict={x: face.test.faces,
                                    y_: face.test.labels,
                                    keep_prob: 1.0})
...
def export_model(input_node_names, output_node_name, input_arrays, output_arrays):
    ...
    converter = tf.lite.TFLiteConverter.from_frozen_graph(
        output_graph_def, input_arrays, output_arrays)
